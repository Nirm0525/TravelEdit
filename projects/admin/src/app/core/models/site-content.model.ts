...
// ---------------------------------------------------------------------------
// Destinos destacados — no duplica datos de destino: guarda solo el heading
// de la sección y una lista ORDENADA de IDs reales de `destinations`. El
// sitio público resuelve esos IDs contra la tabla real (ver
// src/app/core/services/public-destinations.ts). Arranca vacío a propósito:
// hasta que un admin guarde una selección, el público sigue mostrando su
// fallback hardcodeado actual (src/app/core/data/destinations.ts).
...
const DESTINOS_DESTACADOS_LIVE_DEFAULT: DestinosDestacadosContent = {
...
export function toDestinosDestacadosContent(row: SiteContentRow): DestinosDestacadosContent {
  const data = row.content as Partial<DestinosDestacadosContent>;
  return {
    eyebrow: data.eyebrow ?? DESTINOS_DESTACADOS_LIVE_DEFAULT.eyebrow,
    headingLine1: data.headingLine1 ?? DESTINOS_DESTACADOS_LIVE_DEFAULT.headingLine1,
    headingLine2: data.headingLine2 ?? DESTINOS_DESTACADOS_LIVE_DEFAULT.headingLine2,
    headingLine3: data.headingLine3 ?? DESTINOS_DESTACADOS_LIVE_DEFAULT.headingLine3,
    support: data.support ?? DESTINOS_DESTACADOS_LIVE_DEFAULT.support,
    destinationIds: Array.isArray(data.destinationIds) ? data.destinationIds : DESTINOS_DESTACADOS_LIVE_DEFAULT.destinationIds
  };
}
...
const TRAVEL_PROCESS_LIVE_DEFAULT: TravelProcessContent = {
...
export function toTravelProcessContent(row: SiteContentRow): TravelProcessContent {
  const data = row.content as Partial<TravelProcessContent>;
  return {
    titleLine1: data.titleLine1 ?? TRAVEL_PROCESS_LIVE_DEFAULT.titleLine1,
    titleLine2: data.titleLine2 ?? TRAVEL_PROCESS_LIVE_DEFAULT.titleLine2,
    steps: Array.isArray(data.steps) && data.steps.length > 0 ? data.steps : TRAVEL_PROCESS_LIVE_DEFAULT.steps
  };
}
...
const EXPERIENCIAS_LIVE_DEFAULT: ExperienciasContent = {
  eyebrow: 'Experiences',
...
export function toExperienciasContent(row: SiteContentRow): ExperienciasContent {
  const data = row.content as Partial<ExperienciasContent>;
  return {
    eyebrow: data.eyebrow ?? EXPERIENCIAS_LIVE_DEFAULT.eyebrow,
    headingLine1: data.headingLine1 ?? EXPERIENCIAS_LIVE_DEFAULT.headingLine1,
    headingLine2: data.headingLine2 ?? EXPERIENCIAS_LIVE_DEFAULT.headingLine2,
    support: data.support ?? EXPERIENCIAS_LIVE_DEFAULT.support,
    items: Array.isArray(data.items) && data.items.length > 0 ? data.items : EXPERIENCIAS_LIVE_DEFAULT.items
  };
}
...
const THE_EDIT_LIVE_DEFAULT: TheEditContent = {
  eyebrow: 'The Edit',
...
export function toTheEditContent(row: SiteContentRow): TheEditContent {
  const data = row.content as Partial<TheEditContent>;
  return {
    eyebrow: data.eyebrow ?? THE_EDIT_LIVE_DEFAULT.eyebrow,
    headingLine1: data.headingLine1 ?? THE_EDIT_LIVE_DEFAULT.headingLine1,
    headingLine2: data.headingLine2 ?? THE_EDIT_LIVE_DEFAULT.headingLine2,
    supportLine1: data.supportLine1 ?? THE_EDIT_LIVE_DEFAULT.supportLine1,
    supportLine2: data.supportLine2 ?? THE_EDIT_LIVE_DEFAULT.supportLine2,
    ctaLabel: data.ctaLabel ?? THE_EDIT_LIVE_DEFAULT.ctaLabel,
    ctaHref: data.ctaHref ?? THE_EDIT_LIVE_DEFAULT.ctaHref,
    articles: Array.isArray(data.articles) && data.articles.length > 0 ? data.articles : THE_EDIT_LIVE_DEFAULT.articles
  };
}
...
const ABOUT_LIVE_DEFAULT: AboutContent = {
  eyebrow: 'Our Story',
...
export function toAboutContent(row: SiteContentRow): AboutContent {
  const data = row.content as Partial<AboutContent>;
  return {
    eyebrow: data.eyebrow ?? ABOUT_LIVE_DEFAULT.eyebrow,
    titleLine1: data.titleLine1 ?? ABOUT_LIVE_DEFAULT.titleLine1,
    titleLine2: data.titleLine2 ?? ABOUT_LIVE_DEFAULT.titleLine2,
    paragraph1: data.paragraph1 ?? ABOUT_LIVE_DEFAULT.paragraph1,
    paragraph2: data.paragraph2 ?? ABOUT_LIVE_DEFAULT.paragraph2,
    paragraph3: data.paragraph3 ?? ABOUT_LIVE_DEFAULT.paragraph3,
    words: Array.isArray(data.words) && data.words.length > 0 ? data.words : ABOUT_LIVE_DEFAULT.words,
    imageUrl: data.imageUrl ?? ABOUT_LIVE_DEFAULT.imageUrl,
    imageAlt: data.imageAlt ?? ABOUT_LIVE_DEFAULT.imageAlt
  };
}
...
const CTA_FINAL_LIVE_DEFAULT: CtaFinalContent = {
...
export function toCtaFinalContent(row: SiteContentRow): CtaFinalContent {
  const data = row.content as Partial<CtaFinalContent>;
  return {
    eyebrow: data.eyebrow ?? CTA_FINAL_LIVE_DEFAULT.eyebrow,
    titleLine1: data.titleLine1 ?? CTA_FINAL_LIVE_DEFAULT.titleLine1,
    titleLine2: data.titleLine2 ?? CTA_FINAL_LIVE_DEFAULT.titleLine2,
    text: data.text ?? CTA_FINAL_LIVE_DEFAULT.text,
    ctaLabel: data.ctaLabel ?? CTA_FINAL_LIVE_DEFAULT.ctaLabel,
    imageUrl: data.imageUrl ?? CTA_FINAL_LIVE_DEFAULT.imageUrl,
    imageAlt: data.imageAlt ?? CTA_FINAL_LIVE_DEFAULT.imageAlt,
    links: Array.isArray(data.links) && data.links.length > 0 ? data.links : CTA_FINAL_LIVE_DEFAULT.links
  };
}
...
const FOOTER_LIVE_DEFAULT: FooterContent = {
...
// ---------------------------------------------------------------------------
// Secciones personalizadas — a diferencia de las 8 secciones fijas, estas no
// tienen un componente público propio esperándolas: se agregan y se editan
// libremente desde el admin, y el sitio público las renderiza con un bloque
// genérico (eyebrow + título + texto + imagen opcional) al final del Home.
// Todas viven en UNA sola fila de site_content (section_key = 'custom_sections'),
// así que crear una sección nueva nunca requiere ampliar el check constraint
// ni sembrar una fila nueva — solo se agrega un elemento al array.
...
const CUSTOM_SECTIONS_LIVE_DEFAULT: CustomSectionsContent = { sections: [] };

export function toCustomSectionsContent(row: SiteContentRow): CustomSectionsContent {
  const data = row.content as Partial<CustomSectionsContent>;
  return {
    sections: Array.isArray(data.sections) ? data.sections : CUSTOM_SECTIONS_LIVE_DEFAULT.sections
  };
}

export function toFooterContent(row: SiteContentRow): FooterContent {
  const data = row.content as Partial<FooterContent>;
  return {
    exploreHeading: data.exploreHeading ?? FOOTER_LIVE_DEFAULT.exploreHeading,
    explore: Array.isArray(data.explore) && data.explore.length > 0 ? data.explore : FOOTER_LIVE_DEFAULT.explore,
    companyHeading: data.companyHeading ?? FOOTER_LIVE_DEFAULT.companyHeading,
    company: Array.isArray(data.company) && data.company.length > 0 ? data.company : FOOTER_LIVE_DEFAULT.company,
    followHeading: data.followHeading ?? FOOTER_LIVE_DEFAULT.followHeading,
    social: Array.isArray(data.social) && data.social.length > 0 ? data.social : FOOTER_LIVE_DEFAULT.social,
    newsletterHeading: data.newsletterHeading ?? FOOTER_LIVE_DEFAULT.newsletterHeading,
    copyrightText: data.copyrightText ?? FOOTER_LIVE_DEFAULT.copyrightText
  };
}
